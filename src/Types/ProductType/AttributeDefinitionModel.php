...
class AttributeDefinitionModel extends JsonObjectModel implements AttributeDefinition {
    ...
    public function getType()
    {
        if (is_null($this->type)) {
            $value = $this->raw(AttributeDefinition::FIELD_TYPE);
            $this->type = $value;
        }
        return $this->type;
    }
    ...
    public function getName()
    {
        if (is_null($this->name)) {
            $value = $this->raw(AttributeDefinition::FIELD_NAME);
            $value = (string)$value;
            $this->name = $value;
        }
        return $this->name;
    ...
    public function getLabel()
    {
        if (is_null($this->label)) {
            $value = $this->raw(AttributeDefinition::FIELD_LABEL);
            if (is_null($value)) {
    ...
    public function getIsRequired()
    {
        if (is_null($this->isRequired)) {
            $value = $this->raw(AttributeDefinition::FIELD_IS_REQUIRED);
    ...
    public function getAttributeConstraint()
    {
        if (is_null($this->attributeConstraint)) {
            $value = $this->raw(AttributeDefinition::FIELD_ATTRIBUTE_CONSTRAINT);
            $value = (string)$value;
    ...
    public function getInputTip()
    {
        if (is_null($this->inputTip)) {
            $value = $this->raw(AttributeDefinition::FIELD_INPUT_TIP);
            if (is_null($value)) {
                return $this->mapData(LocalizedString::class, null);
            }
            $value = $this->mapData(LocalizedString::class, $value);
    ...
    public function getInputHint()
    {
        if (is_null($this->inputHint)) {
            $value = $this->raw(AttributeDefinition::FIELD_INPUT_HINT);
            $value = (string)$value;
    ...
    public function getIsSearchable()
    {
        if (is_null($this->isSearchable)) {
            $value = $this->raw(AttributeDefinition::FIELD_IS_SEARCHABLE);
