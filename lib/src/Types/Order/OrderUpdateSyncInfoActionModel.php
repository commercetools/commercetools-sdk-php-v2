...
class OrderUpdateSyncInfoActionModel extends OrderUpdateActionModel implements OrderUpdateSyncInfoAction
{
    ...
    public function getChannel()
    {
        if (is_null($this->channel)) {
            $value = $this->raw(OrderUpdateSyncInfoAction::FIELD_CHANNEL);
            if (is_null($value)) {
    ...
    public function getExternalId()
    {
        if (is_null($this->externalId)) {
            $value = $this->raw(OrderUpdateSyncInfoAction::FIELD_EXTERNAL_ID);
            $value = (string)$value;
    ...
    public function getSyncedAt()
    {
        if (is_null($this->syncedAt)) {
            $value = $this->raw(OrderUpdateSyncInfoAction::FIELD_SYNCED_AT);
