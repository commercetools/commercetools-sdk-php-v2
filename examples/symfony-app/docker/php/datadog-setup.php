    ...
    foreach ($selectedBinaries as $command => $fullPath) {
        ...
        foreach ($options['d'] as $cliIniSetting) {
            ...
            foreach ($mainIniFilePaths as $iniFile) {
                ...
                $iniFileContent .= datadog - setup . phpimplode(' = ', $setting) . PHP_EOL;
...
function ini_values($binary)
{
    ...
    if ($found[EXTENSION_DIR] == "") {
        ...
        $found[EXTENSION_DIR] = dirname(PHP_BINARY) . 'datadog-setup.php/' . $found[EXTENSION_DIR];
    }
...
function map_env_to_ini($env)
{
    ...
    switch ($setting[1]) {
        ...
        case 'APPSEC':
            ...
            $ini .= strtolower($setting[1]) . 'symfony-app' . strtolower($setting[2]);
            break;
        default:
